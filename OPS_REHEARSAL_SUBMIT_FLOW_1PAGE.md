...
> ⚠ DEFAULT=EXT(최소 노출)  
> - MODE를 정하지 못하면 EXT로 진행합니다(EXT로 간주).  
> - EXT 금지: 로컬 경로(COPIED_BUNDLE_PATH), 서버명 및 계정명, 내부 커밋 해시  
> - **INT는 팀 내부 인수인계 채널 전용이며 외부 채널 사용 금지**
...
- MODE 미지정이면 **EXT로 간주**(최소 노출)
- EXT 금지: 로컬 경로(COPIED_BUNDLE_PATH), 서버명/계정명, 내부 커밋 해시
- INT 허용: 필요 시 Commit(short) 포함 가능(권장: 파일명 중심)
- INT는 운영자가 MODE=INT를 명시 입력했고 제출 채널이 내부임이 확인된 경우에만 선택합니다. 애매하면 EXT로 진행합니다.
